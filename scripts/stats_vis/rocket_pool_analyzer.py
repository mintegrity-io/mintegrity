#!/usr/bin/env python3
"""
Enhanced script for analysis of addresses using a pre-built transaction graph

How it works:
1. Loads a pre-built graph from a JSON file
2. Extracts addresses which directly interacted with contracts (by default)
   or all addresses from the graph (--all-addresses option)
3. Retrieves enhanced statistics of each address for the last 365 days through Etherscan
   - total volume, max transaction value;
   - total number of interactions with wallets and contracts;
   - wallet creation date;
   - activity patterns, gas fees;
   - daily and monthly activity.
4. Uses existing pricing modules to obtain historical token prices
5. Saves outputs in JSON and CSV format

IMPORTANT: the graph may contain interactions over a limited period but the script gathers statistics over 365 days
"""
...
class WalletStatistics:
    """Extended address statistics for 365 days"""
    address: str
...
class GraphAnalyzer:
    """Generic transaction graph addresses analyzer"""
    ...
    def __init__(self,
        ...
        log.info(f"Initialized Graph Analyzer")
        log.info(f"Graph file: {self.graph_file_path}")
        log.info(f"Output directory: {self.output_dir}")
        log.info(f"Analysis mode: {'All addresses' if analyze_all_addresses else 'Only contract interactions'}")
    ...
    def get_address_type_from_graph(self, graph: TransactionsGraph, address: str) -> str:
        ...
        # First, check in the graph nodes
    ...
    def extract_interacting_addresses(self, graph: TransactionsGraph) -> Set[str]:
        """Extract addresses that directly interact with contract nodes"""

        contract_addresses = {addr for addr, node in graph.nodes.items() if node.type == NodeType.CONTRACT or node.type == NodeType.ROOT}

        if not contract_addresses:
            log.warning("No contract nodes identified in the graph! Will analyze all addresses as fallback...")
        ...
        for edge in graph.edges.values():
            for transaction in edge.transactions.values():
                ...
                if from_addr in contract_addresses or to_addr in contract_addresses:
                    if from_addr not in contract_addresses:
                        interacting_addresses.add(from_addr)
                    if to_addr not in contract_addresses:
                        interacting_addresses.add(to_addr)

        log.info(f"Extracted {len(interacting_addresses)} unique addresses that interact with contracts")
    ...
    def get_wallet_statistics_batch(self, addresses: List[str], graph: TransactionsGraph) -> List[WalletStatistics]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for address in addresses:
                # First try Etherscan (365d)
                future1 = executor.submit(self.get_wallet_statistics_etherscan_365d, address, graph)
                future_to_address[future1] = (address, "etherscan_365d")

            # Gathering results
            address_results = {}

            with tqdm(total=len(addresses), desc="Fetching 365-day address statistics") as pbar:
    ...
    def save_results(self, results: List[WalletStatistics], prefix: str = "addresses_analysis"):
    ...
    def create_summary_report(self, results: List[WalletStatistics], prefix: str, timestamp: str):
        ...
        summary = {
            ...
            "analysis_type": "all_addresses_365d" if self.analyze_all_addresses else "contract_interactions_365d",
            ...
            "analysis_scope": "All addresses in graph (365d analysis)" if self.analyze_all_addresses else "Only addresses that directly interact with contracts (365d analysis)",
        ...
        # log output
        analysis_type_msg = "ALL addresses" if self.analyze_all_addresses else "addresses that interact directly with contracts"
        log.info(f"=== 365-DAY ANALYSIS SUMMARY ===")
    ...
    def run_analysis(self) -> None:
        ...
        log.info("=" * 60)
        log.info("ADDRESS ANALYSIS STARTED")
        log.info("=" * 60)
        ...
        try:
            ...
            if self.analyze_all_addresses:
                ...
                log.info("Analyzing only addresses that directly interact with contracts...")
                addresses_to_analyze = self.extract_interacting_addresses(graph)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Address Analyzer - analyzes 365-day activity of addresses that interact with contracts (by default) or all addresses in graph",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python address_analyzer.py
  python address_analyzer.py --graph-path /path/to/custom_graph.json
  python address_analyzer.py --output-dir /path/to/my_results --max-workers 10
  python address_analyzer.py --all-addresses  # Analyze ALL addresses, not just contract interactions
...
Default paths are calculated relative to the project root directory.
    ...
    parser.add_argument(
        "--all-addresses",
        action="store_true",
        help="Analyze ALL addresses in graph instead of only contract interacting addresses"
    )
