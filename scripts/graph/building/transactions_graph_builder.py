...
class TransactionsGraphBuilder:
    def __init__(self, root_contracts: set[SmartContract], from_time: int, to_time: int):
        ...
        self.processed_addresses: set[tuple[Address, AddressProcessingState]] = set()
    ...
    def mark_address_as_partially_processed(self, address: Address, reason: str):
        self.processed_addresses.add((address, AddressProcessingState.PROCESSED))
        self.addresses_to_process.remove(address)
        log.info(f"Partially processed address: {address}. Total processed: {len(self.processed_addresses)}. Reason: {reason}")

    def is_address_processed(self, address):
        return address in self.processed_addresses

    def get_new_addresses_to_process(self) -> set[Address]:
        all_addresses: set[Address] = set(node.address for node in self.graph.nodes)
        addresses_to_process: set[Address] = set([address for address in all_addresses if not self.is_address_processed(address)])
    ...
    def add_all_interactions_for_address_and_mark_it_as_processed(self, address: Address):
        ...
        if number_of_interactions_to_add <= MAX_TRANSACTIONS_PER_NODE:
            ...
            for interaction_entry in interactions_with_address_capped:
                ...
                self.graph.add_transaction(transaction)
            # Mark processed addresses to avoid infinite loops
            self.mark_address_as_partially_processed(address, "Too many transactions to process. Capped at {}.".format(MAX_TRANSACTIONS_PER_NODE))
